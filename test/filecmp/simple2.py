...
[holderlist.append(os.path.abspath(os.path.join(dir,x))) for x in only_in_one]
[holderlist.append(os.path.abspath(os.path(join(dir,x))) for x in diff_in_one]

if len(dircomp.common_dirs) > 0:    #判断是否存在相同子目录，以便递归
    for item in dircomp.common_dirs:    #递归子目录
        compareme(os.path.abspath(os.path.join(dir1,item)),os.apth.abspath(os.path.join(dir2,item)))
return holderlist

def main():
    if len(sys.argv) > 2:    #要求输入源目录及备份目录
        dir1 = sys.argv[1]
        dir2 = sys.argv[2]
    else:
...
copy_pair = zip(source_files,destination_files)    #将源目录与部分目录文件清单拆分成元组
